...
/// (gml_code, ...arguments)
var n = external_call(global.f_snippet_preproc_run, "", argument[0], "define");

// NB! Store-restore mirrors across snippet_execute_string, snippet_call, snippet_call_ext
// store old arguments:
var _old_argc = global.__snippet__argument_count;
var _old_args = global.__snippet__argument;
var i = 1;
repeat (_old_argc - 1) {
...
var _result = 0;
repeat (n) {
	var _name = external_call(global.f_snippet_preproc_pop_name);
	var _code = external_call(global.f_snippet_preproc_pop_code);
	global.__snippet__result = 0;
...
/// (name, gml_code)
var n = external_call(global.f_snippet_preproc_run, argument0, argument1, "define");
repeat (n) {
	var _name = external_call(global.f_snippet_preproc_pop_name);
	var _code = external_call(global.f_snippet_preproc_pop_code);
...
if (ds_map_exists(global.__snippet__map, _name)) {
	...
	var _enumb = ds_map_find_value(global.__snippet__map, _name);
	event_perform_object(obj_snippets, ev_alarm, _enumb);
	//*/
	
	/* one snippet per object:
	var _obj = ds_map_find_value(global.__snippet__map, _name);
...
/// (name, gml_code)
var _name = argument0;
var n = external_call(global.f_snippet_preproc_run, "properties", argument1, "event");
var _obj = snippet_prepare_object(_name);

// clear old events:
var _events = external_call(global.f_snippet_preproc_concat_names);
if (ds_map_exists(global.__snippet__object_events, _obj)) {
	var _old_events = ds_map_find_value(global.__snippet__object_events, _obj);
	repeat (sniptools_string_split_start(_old_events, chr(10))) {
		var _ename = sniptools_string_split_next();
		var _type = snippet_event_get_type(_ename);
		var _numb = snippet_event_get_number(_ename);
		if (_type == -1) {
			if (_numb != 0) continue;
			// revert properties!
...
repeat (n) {
	var _ename = external_call(global.f_snippet_preproc_pop_name);
	var _code = external_call(global.f_snippet_preproc_pop_code);
	var _type = snippet_event_get_type(_ename);
	var _numb = snippet_event_get_number(_ename);
	if (_type == -1) {
	...
	if (_numb == -2) {
		var _cobj_name = snippet_event_get_number_object();
		var _cobj = snippet_object_get_index(_cobj_name);
		if (_cobj == -1) {
...
var i;
var n = ds_list_size(_files);
var _grid = ds_grid_create(n, 4); // [kind, path, name, meta]

// figure out what to do with each thing:
for (i = 0; i < n; i += 1) {
	var _file = ds_list_find_value(_files, i);
	...
	var _ext = string_lower(filename_ext(_file));
	if (_ext == ".gml") {
		var _noext = filename_change_ext(_file, "");
		var _ext2 = string_lower(filename_ext(_noext));
		switch (_ext2) {
...
// load the code bits:
for (i = 0; i < n; i += 1) {
	var _kind = ds_grid_get(_grid, i, 0);
	var _file = ds_grid_get(_grid, i, 1);
	var _name = ds_grid_get(_grid, i, 2);
	var _meta = ds_grid_get(_grid, i, 3);
	switch (_kind) {
		case "script": case "object":
			var _gml = sniptools_file_get_contents(_pre + _file);
			//show_message(_name + ":" + chr(10) + _gml);
			if (_gml == "") {
				show_error('"' + _file + '" is missing or empty!', false);
			} else if (_kind == "object") {
				var _obj = snippet_define_object(_name, _gml);
				if (is_string(_meta)) {
					if (_meta == "") {
...
/// (path)
var _path = argument0;
var _tf = file_text_open_read(_path);
var _todo = ds_list_create();
var _pre = filename_path(_path);
while (!file_text_eof(_tf)) {
	var _line = file_text_read_string(_tf);
	file_text_readln(_tf);
